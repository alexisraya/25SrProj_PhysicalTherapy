<script lang="ts">
  ...
  function getProgressPercentage(): number {
    ...
    return currentIndex !== -1 ? ((currentIndex + 1) / program.exercises.length) * 100 : 0;
  }
  ...
  function getProjectedStatsIncrease() {
    ...
    if (currentExercise.exerciseType === 'distance') {
      ...
      return {
        ...
        projected:
          stats.totalWeight + adjustedValues.sets * adjustedValues.reps * adjustedValues.weight,
      };
  ...
  async function navigateToNext() {
    ...
    try {
      ...
      const allCompleted = program.exercises.every((ex) => ex.completed || ex.skipped);
  ...
  {:else if program && currentExercise}
    ...
    <div class="exercise-details">
          ...
          <div class="stats-grid">
            {#if currentExercise.exerciseType === 'distance'}
              ...
              <div class="stat-item">
                ...
                <span>
                  {stats.totalDistance + adjustedValues.sets * adjustedValues.steps} steps
                </span>
              </div>
            {:else if currentExercise.exerciseType === 'weight'}
              <div class="stat-item">
              ...
              <div class="stat-item">
                ...
                <span>
                  {stats.totalWeight +
                    adjustedValues.sets * adjustedValues.reps * adjustedValues.weight} lbs
                </span>
              </div>
            {:else if currentExercise.exerciseType === 'time'}
              <div class="stat-item">
              ...
              <div class="stat-item">
                <span>After Completion</span>
                <span>
                  {formatTime(stats.totalTime + adjustedValues.reps * adjustedValues.seconds)}
                </span>
              </div>
            {/if}
          </div>
      ...
      <div class="action-buttons">
        <button class="complete-btn" on:click={handleComplete} disabled={isCompleting}>
          {isCompleting ? 'Completing...' : 'Complete Exercise'}
        </button>
        <button class="skip-btn" on:click={handleSkipExercise}> Skip Exercise </button>
      </div>
    </div>
  {:else}
