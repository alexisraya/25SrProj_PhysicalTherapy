...
<div class="patient-goals-container">
  ...
  {:else if error}
    ...
    <div class="debug-info">
      ...
      {/if}
      <button on:click={loadPatientGoals} class="reload-btn"> Reload Data </button>
    </div>
  {:else if patient}
    <div class="goals-section">
      <p class="goals-hint">Unlock goals as your patient progresses through their recovery.</p>
      ...
      {#if Object.keys(patientGoals).length === 0}
        <div class="empty-goals">
          ...
          <button class="assign-goals-btn" on:click={handleAssignGoals}>
                ...
                <div class="goal-card {goal.unlocked ? 'unlocked' : 'locked'}">
                  <div class="goal-card-header">
                    ...
                    <div class="status-badge-container">
                      <span class="status-badge {goal.unlocked ? 'unlocked' : 'locked'}">
                        {goal.unlocked ? 'Unlocked' : 'Locked'}
                      </span>
                    </div>
                    <button
                      class="toggle-btn {goal.unlocked ? 'unlock-btn' : 'lock-btn'}"
                      on:click={() => toggleGoalLock(goal.goalId, goal.unlocked)}
