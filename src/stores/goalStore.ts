...
import { getUserGoals, updateGoalLockStatus, assignGoalsToUser } from '$firebase/services/milestoneService';
...
function createGoalStore() {
    ...
    return {
        ...
        toggleGoalLock: async (userId: string, goalId: string, currentStatus: boolean) => {
            ...
            try {
                console.log(`Toggling goal ${goalId} from ${currentStatus ? 'unlocked' : 'locked'} to ${!currentStatus ? 'unlocked' : 'locked'}`);
