...
export async function updateGoalLockStatus(userId: string, goalId: string, unlocked: boolean): Promise<void> {
    try {
        console.log(`Updating goal ${goalId} for user ${userId} to ${unlocked ? "unlocked" : "locked"}`);
        ...
        console.log(`Therapist updated goal ${goalId} for user ${userId}: ${unlocked ? "Unlocked" : "Locked"}`);
    } catch (error) {
...
export async function checkAchievements(userId: string): Promise<string[]> {
    try {
        ...
        if (newlyUnlocked.length > 0 || Object.keys(achievementsToUpdate).length !== Object.keys(stats.achievements).length) {
            ...
            console.log(`Updated achievements for user ${userId}, newly unlocked: ${newlyUnlocked.length}`);
        }
