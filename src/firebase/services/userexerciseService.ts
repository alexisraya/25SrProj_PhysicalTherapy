...
export async function completeExercise(
  ...
  try {
    ...
    const [user, program] = await Promise.all([getUser(userId), getCurrentProgram(userId)]);
...
export async function skipExercise(userId: string, exerciseId: string): Promise<void> {
  try {
    await checkAndResetProgress(userId);
