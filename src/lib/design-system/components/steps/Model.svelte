...
<script lang="ts">
  ...
  let selectedModel: number = 1;
  let characters = [
    { id: 1, name: 'Alex', image: Character3, selectable: true },
    { id: 2, name: 'Kieran', image: Character, selectable: false },
    { id: 3, name: 'Bree', image: Character2, selectable: false }
  ];

  // Update completion status - first character is automatically selected
  $: {
    setStepComplete(STEP_INDEX, true);
  }

  function selectCharacter(id) {
    // Only allow selecting the first character
...
<div class="model-step-container">
  ...
  <div class="model-select-container">
    <div class="select-character-section">
      {#each characters as character}
        <button
          class="character {selectedModel === character.id ? 'selected' : ''} {!character.selectable
            ? 'disabled'
            : ''}"
          on:click={() => selectCharacter(character.id)}
        >
          <img
            class="character--img {selectedModel === character.id
              ? 'selected-img'
              : ''} {!character.selectable ? 'disabled-img' : ''}"
            src={character.image}
            alt="Character"
          />
          <div
            class="character--overlay {selectedModel === character.id
              ? 'selected-overlay'
              : ''} {!character.selectable ? 'disabled-overlay' : ''}"
          ></div>
          <div
            class="character--name {selectedModel === character.id
              ? 'selected-name'
              : ''} {!character.selectable ? 'disabled-name' : ''}"
          >
            <p
              style="font-family: {typography.fontFamily.body}; font-size: {typography.fontSizes
                .regular}; font-weight: {selectedModel === character.id
...
<style>
  ...
  /* Styles for disabled/grayed out characters */
  .disabled {
    cursor: not-allowed;
    opacity: 0.5;
    filter: grayscale(70%);
