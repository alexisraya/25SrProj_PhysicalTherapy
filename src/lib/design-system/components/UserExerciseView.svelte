        ...
        {#if $weeklyProgress}
            <div class="weekly-progress">
                ...
                <div class="progress-bar">
                    <div
                        class="progress-fill"
                        style="width: {(($weeklyProgress?.daysCompleted ?? 0) /
                            5) *
                            100}%"
                    ></div>
                </div>
                <p>
                    {$weeklyProgress?.daysCompleted ?? 0}/5 days completed this
                    week
                </p>
                <p>
                    Days needed for streak: {$weeklyProgress?.daysNeededForStreak ??
                        "N/A"}
                </p>
            </div>
        {/if}
            ...
            {#if hasIncompleteExercises}
                {#if isBulkCompleteMode}
                    <button
                        ...
                        disabled={isSaving ||
                            Object.values(selectedExercises).every((v) => !v)}
                    >
            ...
            <div class="exercise-list-section">
                ...
                <div class="exercise-list">
                    {#each $program.exercises as exercise, i (exercise.exerciseId)}
                        <div
                            class="exercise-card {exercise.completed
                                ? 'completed'
                        ...
                        >
                            {#if isBulkCompleteMode && !exercise.completed && !exercise.skipped}
                                <div class="checkbox-container">
                                    <input
                                        type="checkbox"
                                        bind:checked={selectedExercises[
                                            exercise.exerciseId
                                        ]}
                                        on:change={(e) =>
                            ...
                            <div class="exercise-info">
                                ...
                                {#if exercise.equipment}
                                    <small
                                        >Equipment: {exercise.equipment}</small
                                    >
                                {/if}
                            </div>
                                ...
                                {:else if !isBulkCompleteMode}
                                    <div class="exercise-actions">
                                        <button
                                            ...
                                            on:click={() =>
                                                goto(
                                                    `/exploration/firebase-test/exercise-flow/${exercise.exerciseId}`,
                                                )}
                                        >
                                            Start
                                        </button>
