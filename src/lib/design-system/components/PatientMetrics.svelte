<script lang="ts">
  ...
  import {
    ...
    updateRangeOfMotion,
    updateStrength
  } from '$firebase/services/metricsService';
  ...
  {:else}
    ...
    <div class="metrics-container">
      <div class="metric-box">
        ...
        <p class="description">Range of motion (degrees from 0-100).</p>
        <div class="metric-form">
          <div class="form-group">
            <label for="romMonth">Month:</label>
            <input type="number" id="romMonth" bind:value={newRomMonth} min="1" />
          </div>
          <div class="form-group">
            <label for="romDegrees">Degrees (0-100):</label>
            <input type="number" id="romDegrees" bind:value={newRomDegrees} min="0" max="100" />
          </div>
          <button on:click={handleRomSubmit} class="save-btn" disabled={loading}> Save ROM </button>
        </div>
        {#if metricsData && metricsData.rangeOfMotion && metricsData.rangeOfMotion.length > 0}
      ...
      <div class="metric-box">
        <h3>Strength</h3>
        <p class="description">Strength progress (scale from 0-5).</p>
        <div class="metric-form">
          <div class="form-group">
            <label for="strengthMonth">Month:</label>
            <input type="number" id="strengthMonth" bind:value={newStrengthMonth} min="1" />
          </div>
          <div class="form-group">
          ...
          <button on:click={handleStrengthSubmit} class="save-btn" disabled={loading}>
            Save Strength
          </button>
        </div>
        {#if metricsData && metricsData.strength && metricsData.strength.length > 0}
